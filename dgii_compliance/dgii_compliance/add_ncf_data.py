...
def main(doc,method):
	...
	if doc.custom_tipo_de_factura== 'Valor fiscal':
		if doc.name.count('-') == 3:
			...
			ncf_seq_list = frappe.get_all('Secuencia NCF', filters = {'enabled': 1})
			#parent_invoices = frappe.get_all('Sales Invoice', filters = {'status': 'Cancelled'})
			...
			end_ncf = frappe.db.get_value('Secuencia NCF', ncf_seq_list[0], 'end')
			next_ncf = frappe.db.get_value('Secuencia NCF', ncf_seq_list[0], 'next_ncf')
			start_ncf = frappe.db.get_value('Secuencia NCF', ncf_seq_list[0], 'start')
			...
			#set new ncf
			formated_ncf = 100000000 + next_ncf
			...
			if end_ncf == next_ncf: # si se llego al final de la secuencia se deshabilita secuencia de ncf
				frappe.db.set_value('Secuencia NCF', ncf_seq_list[0], 'enabled', 0)
				frappe.db.set_value('Secuencia NCF', ncf_seq_list[0], 'next_ncf', -1)
			else:
